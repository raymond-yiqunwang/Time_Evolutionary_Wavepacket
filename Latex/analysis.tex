...
\begin{figure}[h]
  \center
  \vspace{-1mm}
  \includegraphics[width=0.95\linewidth]{Psi_Fig.png}
...
\begin{figure}[h]
  ...
  \captionsetup{justification=centering}
  \vspace{1mm}
  \includegraphics[width=0.85\linewidth]{rand1}
  \includegraphics[width=0.85\linewidth]{rand2}
  \caption{非本征态含时演化  \label{fig:prop2}}
\end{figure}

上图是使用切比雪夫展开进行的演化演示图，使用对角化方法可以得到类似的结果。然而我们发现，两种展开方法作用在本征态时表现良好，波函数概率分布不发生变化；但是作用在非本征态时波函数的变化似乎不可预测，两者差生一定的差异，我们在后面会讨论非本征态的演化。接下来的结果与分析都是以表现良好的本征态为对象的。
