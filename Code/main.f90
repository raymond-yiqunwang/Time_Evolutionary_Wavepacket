...
contains
  ...
  subroutine diag_kin
    ...
    implicit none
    real*8        :: kin_vec(N,N),kin_evl(N)
    call mat_diag(kin,kin_vec,kin_evl)
    write(*,*) 'kin_evl: ',kin_evl
  end subroutine diag_kin
  
  subroutine diag_v
    use mat_module, only : mat_diag
    implicit none
    real*8        :: V_vec(N,N),V_evl(N)
    call mat_diag(V,V_vec,V_evl)
    write(*,*) 'v_evl',(v_evl)
  end subroutine diag_v
